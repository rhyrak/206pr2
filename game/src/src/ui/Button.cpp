	...
	hitbox{hitbox}, tint{tint}
{
	...
	ImageResizeNN(&temp, ROUND(hitbox.width), ROUND(hitbox.height));
...
int Button::render()
{
	...
	if (hover)
		DrawTexture(texture, ROUND(hitbox.x), ROUND(hitbox.y), tint);
	else
		DrawTexture(texture, ROUND(hitbox.x), ROUND(hitbox.y), WHITE);
